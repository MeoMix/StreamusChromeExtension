define([
    ...
    describe('ClientError', function () {
        ...
        it('Should truncate a long message properly when initializing', function () {
            ...
            expect(clientError.get('message').length).toBeLessThan(256);
        });
        ...
        it('Should throw a validation error when saving too long of a message', function() {
            ...
            expect(clientError.validationError).not.toBeNull();
            expect(clientError.validationError.length).toBeGreaterThan(0);
            expect(clientError.isValid()).toBe(false);
        });
