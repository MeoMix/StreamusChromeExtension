...
define(function () {
	...
	var YouTubePlayerAPI = Backbone.Model.extend({
		...
		load: function () {
			...
			chrome.webRequest.onBeforeSendHeaders.addListener(this._onWebRequestBeforeSendHeaders.bind(this), {
				urls: [this.get('youTubeEmbedUrl')]
			}, ['blocking', 'requestHeaders'] );

			//  This function will be called when the API is fully loaded. Needs to be exposed globally so YouTube can call it.
			window.onYouTubePlayerAPIReady = this._onYouTubePlayerAPIReady.bind(this);

			this._loadExternalYouTubeApi();
		},
